...
- **Treating `inherit-parent` as a model name.** `inherit-parent` means "omit the model field so the subagent inherits the parent chat model." [Setup](./01-setup.md) covers the roles.
