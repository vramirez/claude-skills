...
Transcripts live at `~/.claude/projects/<slug>/<session-uuid>.jsonl`, where `<slug>` is the workspace path with each "/" turned into "-" (so `/Users/you/proj` becomes `-Users-you-proj`). Every line is one chat message.
