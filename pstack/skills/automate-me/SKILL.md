...
Locate the active workspace's transcripts before fanning out. The system prompt names the project's transcript directory under `~/.claude/projects/`. Use only that path. Don't glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects.
